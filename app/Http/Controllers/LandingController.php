...
class LandingController extends Controller
{
    ...
    {
        $maintenance = maintainance::find(1); // Fetch all user pages
        return view('landing.maintenance', compact('maintenance'));
    }
    public function maintain($id)
    {
        $maintenance = maintainance::find($id);

        if (!$maintenance) {
            return redirect()->route('landing')->with('error', 'maintenance not found.');
        }

        return view('landing.mainconfig', compact('maintenance'));
    }
